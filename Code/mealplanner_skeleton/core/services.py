...
def _create_balanced_meal(meal_type: str, target_kcal: int, recent_products: set, excluded_items: list = None):
    ...
    if meal_type == "B":
        ...
        if breakfast_foods and vitamins:
            ...
            # Use max to ensure reasonable minimum portions
            main_amount = max(100, int(target_kcal * 0.7 / main.kcal_100g * 100)) if main.kcal_100g > 0 else 200
            side_amount = max(50, int(target_kcal * 0.3 / side.kcal_100g * 100)) if side.kcal_100g > 0 else 100
        ...
        if proteins and carbs and vitamins:
            ...
            # Use max to ensure reasonable minimum portions, but no upper limit
            protein_amount = max(100, int(target_kcal * 0.4 / protein.kcal_100g * 100)) if protein.kcal_100g > 0 else 150
            carb_amount = max(100, int(target_kcal * 0.4 / carb.kcal_100g * 100)) if carb.kcal_100g > 0 else 150
            vit_amount = max(50, int(target_kcal * 0.2 / vit.kcal_100g * 100)) if vit.kcal_100g > 0 else 100
